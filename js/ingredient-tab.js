			...
			renderHtml += "<div class='list-group col-11'>"
						...
						for(var j=0; j<catagoryJson.length ; j++){
							renderHtml += "<a class='list-group-item list-group-item-action cls_ingredientCont ingredient_"+ catagoryJson[j].id +"'>"
											...
											+ "<label class='btn btn-info btn-md mr-3 col-1 text-center cls_editIngredient' idx='" + catagoryJson[j].id +"' categoryName='"+categoryName+"' data-toggle='modal' data-target='#ingredientModal'>Edit</label>"
	...
	$(document).ready(function(){
		...
		$(".cls_editIngredient, .cls_createIngredient").click(function() {
			   if($(this).hasClass("cls_createIngredient")) {
			       ...
			       var curIngredientObj = _this.getIngredientByIdAndCategory(idx, categoryName);
...
IngredientTab.prototype.getIngredientByIdAndCategory = function(id,ingredientCategoryName) {
	var _this = this;
	var toRet = null;
	$.each( _this.ingredientJson, function( categoryName, ingredientsArr ) {
		$.each(ingredientsArr, function( index, ingredientObj ) {
			  if((ingredientObj.id).toString() == id && categoryName == ingredientCategoryName) {
